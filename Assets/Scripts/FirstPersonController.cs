...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        playerInput.deviceLostEvent.AddListener((x) => Debug.Log("Reconectar el mando, por favor")); // esto es un mensaje opcional por si se desconecta el mando

        //playerInput.SwitchCurrentActionMap("Coche");
    }
    ...
    private void Saltar(InputAction.CallbackContext obj)
    {
        if (IsGrounded())
        {
            ...
            verticalMovement.y = math.sqrt(-2 * gravityScale * jumpHeight);
        }
    }
